...
export default function UsersScreen() {
  ...
  const [functions, setFunctions] = useState<
    Array<{ id_funcion: number; n_funcion: string }>
  >([]);
  ...
  const fetchFunctions = async () => {
    try {
      const fetchedFunctions =
        await apiService.get<Array<{ id_funcion: number; n_funcion: string }>>(
          "/funciones"
        );
      ...
      <View className="bg-white mb-3 rounded-lg shadow-sm overflow-hidden">
        <View className="p-4">
          ...
          <View className="flex-row justify-between items-center">
            ...
            >
              <Text className="text-white text-xs font-medium">
                {item.funcion?.n_funcion || "Sin función"}
              </Text>
            </View>
          </View>
          ...
          <Text className="text-gray-600 text-sm mt-1">
            {item.area?.n_area || "Sin área asignada"}
          </Text>
            ...
            {item.telefono_consultorio && (
              <View className="flex-row items-center mt-1">
                ...
                <Text className="text-gray-700 ml-2">
                  Consultorio: {item.telefono_consultorio}
                </Text>
              </View>
            )}
          </View>
  ...
  const renderFunctionCard = (func: {
    ...
    const userCount = users.filter(
      (u) => u.funcion?.id_funcion === func.id_funcion
    ).length;
  ...
  if (error) {
    return (
      <View className="flex-1 items-center justify-center bg-background-color p-4">
        ...
        <Text className="text-gray-600 mt-2 mb-6 text-center">{error}</Text>
        <TouchableOpacity
      ...
      {isFiltersView ? (
        ...
        <View className="flex-1">
          ...
          <View className="flex-row items-center mb-4">
            ...
            <Text className="text-title-color text-lg font-bold">
              {functions.find((f) => f.id_funcion === selectedFunctionId)
                ?.n_funcion || "Usuarios"}
            </Text>
          </View>
