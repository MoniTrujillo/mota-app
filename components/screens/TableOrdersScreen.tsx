...
const statusStyle = (statusRaw: string | undefined) => {
  ...
  if (s.includes('confirm')) return { bg: 'bg-green-100', dot: 'bg-green-500', text: 'text-green-700', label: 'Confirmado' };
  if (s.includes('error')) return { bg: 'bg-red-100', dot: 'bg-red-500', text: 'text-red-700', label: 'Error' };
  if (s.includes('paus')) return { bg: 'bg-yellow-100', dot: 'bg-yellow-500', text: 'text-yellow-700', label: 'Pausado' };
  if (s.includes('termin')) return { bg: 'bg-gray-200', dot: 'bg-gray-500', text: 'text-gray-700', label: 'Terminado' };
...
export default function TableOrdersScreen({ onEditPedido }: TableOrdersScreenProps) {
  ...
  const fetchPedidos = async () => {
    ...
    try {
      ...
      const mapped: Pedido[] = list.map((p: any) => ({ id_pedido: p.id_pedido ?? p.id ?? 0, estatus: p.estatus }));
      setPedidos(mapped);
    } catch (e: any) {
  ...
  <ScrollView contentContainerStyle={{ paddingLeft: 24, paddingRight: 0, paddingBottom: 24 }}>
        {loading && (
        ...
        {!loading && !error && pedidos.map((p, idx) => {
          ...
          return (
            <View key={`${p.id_pedido}-${idx}`} className="py-3">
              <View className="flex-row items-center">
                ...
                <View className={`flex-row items-center ${st.bg} px-2 py-1 rounded-full flex-[1] self-start ml-2`}> 
