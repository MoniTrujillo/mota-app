  ...
  const loadData = async () => {
    ...
    try {
      ...
      const mapped: PedidoUI[] = onlyDelivery.map((p) => {
        ...
        let estatusPago =
          p.estatuspago?.n_estatuspago ||
          (p as any).estatus_pago?.n_estatuspago ||
          "";
        if (!estatusPago) {
            ...
            return (
              ...
              >
                ...
                <View className="bg-green-100 px-4 py-3 flex-row items-center justify-between">
                  ...
                  {!!it.prioridad && (
                    <View className={`px-2 py-1 rounded-full ${pStyle.bg}`}>
                      <Text className={`${pStyle.text} text-xs font-semibold`}>
                        {it.prioridad}
                      </Text>
                    </View>
                  )}
                ...
                <View className="px-4 py-3 flex-row justify-between items-start border-t border-gray-200">
                  <View>
                    {!!it.fechaEntrega && (
                      <Text className="text-title-color">
                        {it.fechaEntrega}
                      </Text>
                    )}
                  </View>
          ...
          <View className="bg-white rounded-lg w-11/12 max-w-2xl max-h-5/6">
            ...
            <View className="flex-row justify-between items-center p-4 border-b border-gray-200">
              <Text className="text-xl font-bold text-title-color">
                Dirección - Pedido #{selectedPedidoDetails?.id_pedido}
              </Text>
              <TouchableOpacity
                onPress={() => {
                  setShowModal(false);
              ...
              <ScrollView className="p-5">
                ...
                <View className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  <Text className="text-title-color text-lg font-bold">
                    {selectedPedidoDetails.direccion || "Sin dirección"}
                  </Text>
                </View>
